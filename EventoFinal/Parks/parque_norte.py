...
def run():
    ...
    for t in times:
        ...
        for signal in range(lim):
            ...
            try:
                total = 150
                free = random.randint(0, 20)
                message = json.dumps({
                        "timestamp": s.isoformat(),
                        "fields": {
                            "free_places" : 0 if ctl % 3 == 2 and signal == lim - 1 else free,  ### dados simulados
                            "full_places" : total if ctl % 3 == 2 and signal == lim - 1 else (total - free)  ### dados simulados
                        }
                    })
