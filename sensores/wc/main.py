#https://erich.forler.ca/component/content/article/33-blog-hands-on-tech/146-raspberry-pi-ultrasonic-distance-sensor-hr-srf05-full-lesson?Itemid=101
...
publisher = SendMessage('queue17') # init publish, passing queue

GPIO.add_event_detect(echo_pin, GPIO.BOTH, callback=timer_call)  # add rising and falling edge detection on echo_pin (input)

empty_distance = round(check_distance(), 1) # empty wc
occupacity_aux = True

# init with wc free
send(1)
#print('1º ' + str(empty_distance))

while True:
    new_distance = round(check_distance(), 1)

    # the variation when the state is the same is 0.1
    # so if empty_distance is greater then the new_distance and the previous state is free = true
    # then the wc is occupied
    if (empty_distance-1 > new_distance) and occupacity_aux:
        send(0)
        occupacity_aux = False

    # if empty_distance is smaller then new_distance and the previous state is full = false
    # then the wc is free again
    elif (empty_distance < new_distance+1) and not occupacity_aux:
        send(1)
        occupacity_aux = True
